...
export const Wheel = ({
  ...
  const setStartingOption = useCallback((optionIndex: number, optionMap: number[][]) => {
  ...
  }, [data, backgroundColors, textColors, fontFamily, setStartingOption, startingOptionIndex, fontSize, fontWeight, fontStyle, rouletteUpdater])

  useEffect(() => {
    if (mustStartSpinning && !isCurrentlySpinning && prizeMap.length > 1) {
      setIsCurrentlySpinning(true)
      startSpinning()
      const selectedPrize =
          prizeMap[prizeNumber][0]
  ...
  }, [isCurrentlySpinning, mustStartSpinning, prizeMap, prizeNumber, startSpinning])
